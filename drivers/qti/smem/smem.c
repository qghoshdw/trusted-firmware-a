...
int qti_smem_init(void)
{
	...
	if (ret != 0) {
		ERROR("smem: failed to map BOOT info: %d\n", ret);
		return ret;
	}

	va = qti_smem_plat_get_addr(0U);
	if (va == NULL) {
		ERROR("smem: BOOT info not mapped\n");
		return -EIO;
	}
	...
	ret = qti_smem_plat_map(toc_offset, QTI_SMEM_TOC_SIZE,
				QTI_SMEM_PLAT_MAP_RO);
	if (ret != 0) {
		ERROR("smem: failed to map TOC: %d\n", ret);
		return ret;
	}

	va = qti_smem_plat_get_addr(toc_offset);
	if (va == NULL) {
		ERROR("smem: TOC not mapped\n");
		return -EIO;
	}
